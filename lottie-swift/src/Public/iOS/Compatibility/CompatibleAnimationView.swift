...
public final class CompatibleAnimation: NSObject {
  ...
  @objc
    public init(name: String = "", bundle: Bundle = Bundle.main, jsonStr: String = "") {
    self.name = name
    self.bundle = bundle
    ...
    @objc
    static func fromJsonStr(_ jsonStr: String) -> CompatibleAnimation  {
        return CompatibleAnimation(jsonStr: jsonStr)
    }

  internal var animation: Animation? {
    if(!jsonStr.isEmpty) {
        return Animation.fromJson(jsonStr)
    }
